-- Adds previous_closed_month to the averages block of _compute_euribor_stats().
-- The prev_closed_month CTE already existed; this adds month_start and surfaces
-- both fields in averages so the frontend can render "vs <mes>" without extra math.
...
prev_closed_month AS (
    SELECT DATE_TRUNC('month', CURRENT_DATE - INTERVAL '2 months')::date AS month_start,
           ROUND(AVG(rate)::numeric, 3) AS val
    FROM euribor_rates
...
SELECT jsonb_build_object(
    ...
    'averages', jsonb_build_object(
        ...
        'previous_closed_month', CASE
            WHEN pcm.val IS NOT NULL
            THEN jsonb_build_object('month', pcm.month_start, 'rate', pcm.val)
            ELSE NULL
        END,
        'quarterly',         aq.val,
        'ytd',               ay.val,
