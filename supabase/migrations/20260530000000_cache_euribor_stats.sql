-- Crea la tabla de caché solo si no existe (seguro si ya está creada)
  CREATE TABLE IF NOT EXISTS euribor_stats_cache (
  ...
  -- Función interna de cómputo (actualizada con yearly_series)
          ...
          'daily_series', (
              ...
              FROM (
                  SELECT rate_date, rate, rn,
                      ROUND(AVG(rate) OVER (ORDER BY rate_date ROWS BETWEEN 6  PRECEDING AND CURRENT ROW)::numeric, 3)
  AS ma7,
                      ROUND(AVG(rate) OVER (ORDER BY rate_date ROWS BETWEEN 29 PRECEDING AND CURRENT ROW)::numeric, 3)
  AS ma30
                  FROM (
                      SELECT rate_date, rate,
                             ROW_NUMBER() OVER (ORDER BY rate_date DESC) AS rn
  ...
  -- Función pública (SECURITY DEFINER para que anon pueda leer la caché)
